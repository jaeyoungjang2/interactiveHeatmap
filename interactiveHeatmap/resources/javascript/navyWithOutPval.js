function get_information(gene){
  ...
  for (var key in readJson.data.nodes) {
    ...
    if(geneID && geneID[0] == gene){
      ...
      nav_changer(gene, gene_Symbol, description, gene_biotype, fold_change);
    }
  }
  // return [gene_Symbol, description, gene_biotype];
}

function nav_changer(gene, gene_Symbol, description, gene_biotype, fold_change){
  ...
  const navFoldChange = document.querySelector(".fold_change");
  ...
  navFoldChange.innerHTML = fold_change
}
